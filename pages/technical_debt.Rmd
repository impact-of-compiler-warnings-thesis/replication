...
```{r td-determine-natural-scale-means, message=FALSE}
summary(td1)

td1_std_means <- c(0.26, 0.18,-0.08,-0.21, 0.23,-0.17,-0.30,-0.15,-0.30,-0.30,-0.18)
determine_natural_scale_means(td1_std_means, df$sqale_debt_ratio)
```
...
```{r td-warnings-no-warnings , fig.width=6, fig.height=2.8}
...
## Without outliers
```{r td-without-outliers, fig.width=6, fig.height=2.8, message=TRUE}
...
# Fit TD1 using the trimmed data
td1_no_outliers <-fit_model_td1("td1_no_outliers", data_no_outliers)
summary(td1_no_outliers)

td1_culled_std_means <- c(0.38, 0.29, -0.04, 0.21, -0.12, -0.15, -0.31, -0.15, -0.33, -0.32, -0.20)
determine_natural_scale_means(td1_culled_std_means, culled_df$sqale_debt_ratio)


# Plot Major categories without outliers
plot_intervals_of_major_categories(td1_no_outliers) + 
  labs(title="TD1 (no outliers)", x="Technical Debt Ratio (std)")
...
td1_no_outliers_factor_uses_warnings <-
...
plot_intervals_of_categories(td1_no_outliers_factor_uses_warnings) +
   labs(title="TD1 (no outliers)", x="Technical Debt Ratio (std)") +
